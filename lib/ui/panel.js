...
import { isGroup, isPlain } from "../messages.js";
...
export function createPanel(toast) {
            ...
            if (group) {
                openEntryEditor({
                    ...
                    onSave: async (input) => {
                        ...
                        try {
                            await api.updateEntry(curId(), e.id, { name: input.name, role: input.role });
                            await refreshAll();
                            toast('已保存');
                        }
                        catch (err) {
    ...
    function bindTopDrag(w, _e) {
        ...
        attachDrag({ handle: h, item: w, container: listBox, onDrop: (items) => {
                ...
                if (ids.length !== state.topOrder.length)
                    return;
                if (ids.join('|') === state.topOrder.join('|'))
                    return;
