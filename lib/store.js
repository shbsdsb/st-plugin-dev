...
const VALID_KINDS = ['plain', 'grouped'];
const MAX_BLOCKS = 50;
...
function cleanKind(v) {
    ...
    if (!VALID_KINDS.includes(k))
        throw new Error('kind 非法,仅支持 plain/grouped');
    return k;
}
...
function cleanEntryInput(b) {
    ...
    if (kind === 'plain' && blocks.length > 0)
        throw new Error('普通条目不能包含内容块');
...
function parseEntry(raw) {
    ...
    const blocks = Array.isArray(e.blocks)
        ...
        : [];
    const kind = VALID_KINDS.includes(e.kind)
        ? e.kind
...
export function createStore(persist) {
    return {
        ...
        async createEntry(formId, input) {
            ...
            const entry = { id: entryId, name: clean.name, role: clean.role, text: clean.text, kind: clean.kind, blocks: clean.blocks };
        ...
        async updateEntry(formId, entryId, input) {
            ...
            if (!existing)
                throw new NotFoundError('条目不存在');
            await persist.write(entryFile(formId, entryId), { id: entryId, name: clean.name, role: clean.role, text: clean.text, kind: clean.kind, blocks: clean.blocks });
        },
