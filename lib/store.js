...
export function makeTitle(content, max = 20) {
    const flat = seg(content).replace(/\s+/g, ' ');
    const chars = Array.from(flat);
    return chars.slice(0, max).join('');
}
export function createMultiStore(db) {
    ...
    function toMessage(r) {
        return { id: Number(r.id), role: String(r.role), content: String(r.content), createdAt: String(r.created_at) };
    }
    return {
        listSessions() {
        ...
        appendMessage(sessionId, role, content) {
            ...
            if (!['system', 'user', 'assistant'].includes(role))
                throw new Error('非法消息角色');
            const row = db.prepare('SELECT title, updated_at FROM sessions WHERE id = ?').get(sessionId);
            ...
            if (role === 'user' && row.title === '新会话') {
                ...
                if (title !== '')
                    db.prepare('UPDATE sessions SET title = ?, updated_at = ? WHERE id = ?').run(title, t, sessionId);
                else
