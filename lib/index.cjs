...
function createPanel(toast) {
  ...
  async function createPlain(cur) {
    try {
      const { entryId } = await createEntry(cur.id, { name: "\u65B0\u6761\u76EE", role: "user", text: "", kind: "plain", blocks: [] });
      await renderAll();
      openEntryEditor({
        ...
        onSave: async (input) => {
          ...
          try {
            await updateEntry(cur.id, entryId, { ...input, kind: "plain", blocks: [] });
            await renderAll();
            toast("\u5DF2\u4FDD\u5B58\u6761\u76EE");
          } catch (e) {
            toastError(e);
  ...
  async function createGrouped(cur) {
    try {
      const { entryId } = await createEntry(cur.id, { name: "\u65B0\u6761\u76EE", role: "user", text: "", kind: "grouped", blocks: [] });
  ...
  function renderEntryWrap(e, formId) {
    ...
    const caretBtn = button("prp text-btn", "", () => {
      if (grouped) void doToggleExpand(e.id);
    });
    caretBtn.textContent = grouped ? expanded ? "\u6536\u8D77" : "\u5C55\u5F00" : "";
    if (!grouped) caretBtn.style.visibility = "hidden";
      ...
      openEntryEditor({
        ...
        onSave: async (input) => {
          ...
          try {
            ...
            await updateEntry(formId, e.id, { ...input, kind: latest?.kind ?? "plain", blocks: latest?.blocks ?? [] });
            await renderAll();
            toast("\u5DF2\u4FDD\u5B58\u6761\u76EE");
          } catch (err) {
            toastError(err);
        ...
        if (h) {
          attachDrag({
            ...
            onDrop: (items) => {
              ...
              void (async () => {
                try {
                  await updateEntry(formId, e.id, { name: latest.name, role: latest.role, text: latest.text, kind: latest.kind, blocks });
                  await renderAll();
                  toast("\u5DF2\u4FDD\u5B58\u5185\u5BB9\u5757\u987A\u5E8F");
                } catch (err) {
                  toastError(err);
  ...
  function renderBlockRow(e, formId, b) {
    ...
    const saved = () => {
      ...
      void (async () => {
        ...
        try {
          await updateEntry(formId, e.id, { name: latest.name, role: latest.role, text: latest.text, kind: latest.kind, blocks: next });
          await renderAll();
          toast("\u5DF2\u4FDD\u5B58\u5185\u5BB9\u5757");
        } catch (err) {
          ta.value = b.text;
      ...
      confirmDialog({
        ...
        onOk: () => {
          void (async () => {
            ...
            if (!latest) return;
            try {
              await updateEntry(formId, e.id, { name: latest.name, role: latest.role, text: latest.text, kind: latest.kind, blocks: latest.blocks.filter((x) => x.id !== b.id) });
              await renderAll();
              toast("\u5DF2\u5220\u9664\u5185\u5BB9\u5757");
            } catch (err) {
              toastError(err);
  ...
  async function addBlock(entryId, formId) {
    ...
    try {
      await updateEntry(formId, entryId, { name: latest.name, role: latest.role, text: latest.text, kind: latest.kind, blocks: [...latest.blocks, { id: bid, text: "" }] });
      await renderAll();
      toast("\u5DF2\u6DFB\u52A0\u5185\u5BB9\u5757");
    } catch (err) {
      toastError(err);
