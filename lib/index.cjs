...
function createConfigPanel(ui, api) {
  ...
  async function openModels() {
    ...
    try {
      const list = key ? await api.fetchModelsByInput({ format: state.format, baseUrl: state.baseUrl, apiKey: key }) : state.id !== null ? await api.fetchModels(state.id) : (() => {
  ...
  async function runTest() {
    ...
    try {
      const ok = await (chk.mode === "id" ? api.testPreset({ id: state.id }) : api.testPreset({ format: state.format, baseUrl: state.baseUrl, model: state.model, apiKey: key }));
