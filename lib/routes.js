import { listPresets, getPreset, createPreset, updatePreset, deletePreset, getActivePresetId, setActivePresetId, clearIfActivePreset } from "./db.js";
...
export function registerRoutes(register, dep) {
    ...
    add('exact', '/api/llm/test', async (req, res) => {
        try {
            const b = JSON.parse(await readBody(req));
            let format, baseUrl, key, model, timeout = 30;
            if (b.id) {
                const p = getPreset(db, Number(b.id));
                if (!p)
                    return fail(res, '预设不存在');
                ...
                timeout = p.timeout;
                if (!model)
                    return fail(res, '该预设未保存模型,请先填写并保存');
            }
            else {
                format = String(b.format ?? '');
                baseUrl = String(b.baseUrl ?? '');
                ...
                model = String(b.model ?? '');
                timeout = Number(b.timeout) || 30;
                if (!format || !baseUrl || !model || !key)
                    return fail(res, '请提供 format/baseUrl/model/apiKey(或已保存的 id)');
            }
    ...
    add('exact', '/api/llm/active', async (req, res) => {
        try {
            if (req.method === 'GET')
                return ok(res, { id: getActivePresetId(db) });
            const b = JSON.parse(await readBody(req));
            const id = Number(b.id);
            if (!Number.isFinite(id) || !getPreset(db, id))
                return fail(res, '预设不存在');
