...
export function createConfigPanel(ui: UiToolsLike, api: typeof import('./api.ts')): HTMLElement {
  ...
  const fetchBtn = el('button', 'model-fetch-btn'); const arrow = el('span', 'arrow'); arrow.textContent = '▼'; const label = el('span'); label.textContent = '拉取模型'; fetchBtn.append(arrow, label)
