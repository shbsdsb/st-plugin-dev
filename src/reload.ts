...
import type { TreeBuilderService } from './types.ts'

export interface ReloadContext extends Context {
  treeBuilder: TreeBuilderService
}

/** 保守热更新:新树构建 + 变更插件隔离验证全部通过 → loader 替换;任一失败保持旧状态 */
export async function reload(ctx: ReloadContext, changed: string[]): Promise<void> {
  ...
  try {
    ...
    ctx.logger.warn(`[hot-reload] 新树构建失败,保持旧插件: ${(error as Error).message}`)
    return
  }
  ...
  for (const id of changed) {
    ...
    try {
      ...
      ctx.logger.warn(`[hot-reload] ${id} 隔离验证失败,保持旧插件: ${(error as Error).message}`)
      return
    }
  }
  try {
    ...
    ctx.logger.warn(`[hot-reload] loader 更新失败: ${(error as Error).message}`)
  }
}
