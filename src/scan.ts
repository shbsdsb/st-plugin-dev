...
import { checkImmedConsistency, topoSort } from './topo.ts'
import type { ClientBootEntry, StClient } from './types.ts'
...
interface ProfileManifest {
  st?: { profile?: (string | { name: string; file: string })[] }
}
...
function resolveBundleDir(stHome: string, entry: string | { name: string; file: string }): string | undefined {
  if (typeof entry === 'string') return join(stHome, 'node_modules', entry)
 ...
 */
export function scanBundleDirs(stHome: string, profile?: string): Map<string, string> {
  const profileName = profile ?? 'default'
  const profilePkg = readJson<ProfileManifest>(join(stHome, 'profile', profileName, 'package.json'))
...
export async function scanBundles(opts: ScanOptions): Promise<ClientBootEntry[]> {
 ...
 */
export function scanBundlesSync(opts: ScanOptions): ClientBootEntry[] {
  const warn = opts.warn ?? (() => {})
  const profile = opts.profile ?? 'default'
  const profilePkg = readJson<ProfileManifest>(join(opts.stHome, 'profile', profile, 'package.json'))
  const profileList = profilePkg?.st?.profile ?? []
  ...
  for (const item of profileList) {
    ...
    if (!dir) continue
    const pkg = readJson<BundleManifest>(join(dir, 'package.json'))
    const client = pkg?.st?.client
    if (!client || client.kind !== 'web') continue
  ...
  for (const w of warnings) warn(`[client-find] ${w}`)
  const byId = new Map(entries.map((e) => [e.id, e]))
  const sorted = order.map((id) => byId.get(id)!).filter(Boolean)

  // 依赖-立即性一致性警告
  for (const w of checkImmedConsistency(sorted)) warn(`[client-find] ${w}`)
