...
export function parseListeningPids(output: string, port: number): number[] {
  const pids: number[] = []
  const re = /^\s*TCP\s+(\[[^\]]+\]|\S+):(\d+)\s+\S+\s+LISTENING\s+(\d+)\s*$/i
  for (const line of output.split(/\r?\n/)) {
    const m = line.match(re)
    if (m && Number(m[2]) === port) pids.push(Number(m[3]))
  }
  return [...new Set(pids)]
}
...
/** 强杀指定 PID;失败抛 Error */
export function taskkillPid(pid: number): void {
