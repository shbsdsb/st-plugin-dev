// agent_plugin_dev/client-find/src/topo.ts
...
export function topoSort(items: TopoItem[]): TopoResult {
...
export function checkImmedConsistency(entries: ClientBootEntry[]): string[] {
  const warnings: string[] = []
  const immedBy = new Map(entries.map((e) => [e.id, e.immed]))
  for (const entry of entries) {
    if (!entry.immed) continue
