...
import type { ClientBootEntry } from './types.ts'

export type { ClientBootEntry }
...
export function resolveSeed(spec: string, table: Record<string, string>): string {
...
export function executeCjs(src: string, requireFn: (spec: string) => unknown): Record<string, unknown> {
...
export function buildLoaderSource(table: Record<string, string>, seedNames: string[], boot: ClientBootEntry[]): string {
  ...
  // Node 内置模块浏览器空实现:打包时 external 的 node: 模块在浏览器 require 返回空对象,
  // 满足 cordis 等框架顶层 import(浏览器端不调用其 Node API 函数)。
  for (const n of ['node:fs', 'node:fs/promises', 'node:path', 'node:url']) seedCache.set(n, {})
