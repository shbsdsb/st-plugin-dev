...
export function parsePatch(content: string, source: string): PatchEntry[] {
  const data: unknown = yaml.load(content, { schema: yaml.JSON_SCHEMA }) ?? []
  if (!Array.isArray(data)) throw new Error(`cordis.patch.yml 顶层必须为数组: ${source}`)
  return data.map((item, i) => {
    ...
    const entry: PatchEntry = { id: rec.id }
...
/** 读取 patch 文件;不存在 → [] */
export async function readPatch(path: string): Promise<PatchEntry[]> {
...
export function applyEntries(
  ...
  for (const { id, config } of entries) {
    ...
    if (idx >= 0) {
      ...
      if (hasConfig) target.config = config
      else delete target.config
      result[idx] = target
    } else {
      const entry: PatchEntry = { id }
      if (hasConfig) entry.config = config
      result.push(entry)
    }
  }
  return result
...
export function extractHeaderComments(content: string): string {
...
export async function writePatch(path: string, list: PatchEntry[]): Promise<void> {
